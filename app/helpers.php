...
if (!function_exists('fct_cdn'))
{
    function fct_cdn($path, $hasFullUrl = false, $hasEnd=true)
    {
        ...
        return rtrim($domain, '/') . $path . ($hasEnd?'?_rd=201807201140': '');
    }
}
...
if (!function_exists('wechat_share'))
{
    function wechat_share($share)
    {
        ...
        $title = str_replace(PHP_EOL, "", $share['title']);
        $link = str_replace(PHP_EOL, "", $share['link']);
        $img = str_replace(PHP_EOL, "", $share['img']);
        $desc = str_replace(PHP_EOL, "", $share['desc']);
...
if (!function_exists('gen_qrcode'))
{
    ...
    {
        return 'https://api.fangcun.com/image/genqrcode?size=200&text=' . $url;
    }
}
